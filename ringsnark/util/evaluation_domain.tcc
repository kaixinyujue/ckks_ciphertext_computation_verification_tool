...
evaluation_domain<RingT>::evaluation_domain(const size_t m)
  ...
  for (uint64_t i = 0; i < m; i++) {
    ...
    lagrange_denominator.resize(m);
//  for(size_t i=0;i<m;i++){
//      lagrange_denominator[i]=RingT::one();
//      for(size_t j=0;j<m;j++){
//          if (i != j)
//          lagrange_denominator[i] *= (values[i] - values[j]);
//      }
//  }
}
...
template <typename RingT>
vector<RingT> evaluation_domain<RingT>::pre_evaluate_all_lagrange_polynomials(
        const RingT &t) const {
...
//  }
//   std::cout<<'evaluation_domain.tcc'<<std::endl;
    //12-28 优化:预计算
    RingT p=RingT::one();
    for(int j=0;j<m;j++)
    {
    ...
    for(int j=0;j<m;j++){
        lagrange[j]=p/(t-values[j])/lagrange_denominator[j];
    }
    return lagrange;
}
