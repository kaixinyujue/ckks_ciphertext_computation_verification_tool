...
class evaluation_domain {
 protected:
  vector<RingT> values;
  vector<RingT> lagrange_denominator;
 ...
 public:
  ...
  vector<RingT> get_values(){
    return values;
  };

  vector<RingT> get_lagrange_denominator(){
    return lagrange_denominator;
  };

  void set_values(vector<RingT> &v0){
    size_t vs = v0.size();
    values.resize(vs);
  ...
  void set_lagrange_denominator(vector<RingT> &ld0){
    size_t lds = ld0.size();
    lagrange_denominator.resize(lds);
    for(size_t i=0;i<lds;i++){
      lagrange_denominator[i] = ld0[i];
    }
  };
  ...
  std::vector<RingT> pre_evaluate_all_lagrange_polynomials(const RingT &t) const;
  /**
