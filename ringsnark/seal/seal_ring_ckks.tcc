...
namespace ringsnark::seal {
    ...
    bool RingElem::is_poly()  { return std::holds_alternative<Poly>(value); }

    bool RingElem::is_scalar()  {
        return std::holds_alternative<Scalar>(value);
    }

    RingElem::Poly RingElem::get_poly()  { return std::get<Poly>(value); }
...
//    }
    ...
    bool RingElem::is_invertible()  noexcept {
        if (is_scalar()) {
    ...
    RingElem &RingElem::operator+=( RingElem &other) {
        if (is_poly()) {
            if (other.is_poly()) {
    ...
    bool operator==( RingElem &lhs,  RingElem &rhs) {
    ...
    std::ostream &operator<<(std::ostream &out,  RingElem &elem) {
        if (elem.is_scalar()) {
    ...
    std::vector<EncodingElem> EncodingElem::encode(
             SecretKey &sk,  std::vector<RingElem> &rs) {
...
#pragma omp parallel for default(none) \
    shared(rs, encs, encryptors, ::seal::parms_id_zero)
        for (int i = 0; i < rs.size(); i++) {
            ...
            for (size_t i = 0; i < get_contexts().size(); i++) {
                ...
                encoders[i]->encode(poly.get_limb(i),pow(2.0,48),ptxt);
    ...
    EncodingElem EncodingElem::inner_product(
            vector<EncodingElem>::iterator a_start,
            vector<EncodingElem>::iterator a_end,
            vector<RingElem>::iterator b_start,
            vector<RingElem>::iterator b_end) {
  ...
  for (size_t i = 1; i < EncodingElem::get_contexts().size(); i++) {
    ...
    RingElem EncodingElem::decode(SecretKey &sk, EncodingElem &e) {
        ::seal::Plaintext ptxt;
        ...
        for (size_t i = 0; i < e.ciphertexts.size(); i++) {
            ...
            vector<double> curr_limb(parms.poly_modulus_degree());
    ...
    EncodingElem &EncodingElem::operator+=(EncodingElem &other) {
        ...
        for (size_t i = 0; i < this->ciphertexts.size(); i++) {
           ...
           // printf("+=:%d/%d\n", i, this->ciphertexts.size() - 1);
            try {
                //cout<<"+= scale before "<<i<<"  "<<log2(this->ciphertexts[i].scale())<<" "<<log2(other.ciphertexts[i].scale())<<endl;
                this->ciphertexts[i].scale()=other.ciphertexts[i].scale();
                //other.ciphertexts[i].scale()=pow(2.0,48);
                auto index1=get_contexts()[i].get_context_data(this->ciphertexts[i].parms_id())->chain_index();
                auto index2=get_contexts()[i].get_context_data(other.ciphertexts[i].parms_id())->chain_index();
               // if(index1>=index2){
                  //  evaluators[i]->mod_switch_to_inplace(this->ciphertexts[i],other.ciphertexts[i].parms_id());
                //}else{
                 //   evaluators[i]->mod_switch_to_inplace(other.ciphertexts[i],this->ciphertexts[i].parms_id());
               // }
    ...
    EncodingElem &EncodingElem::operator*=(RingElem &r) {
        ::seal::Plaintext ptxt;
        ...
        if (r.is_scalar()) {
            ...
            cout<<"this->ciphertexts.size() "<<this->ciphertexts.size()<<endl;
//            auto coeff=get_contexts()[0].first_context_data()->parms().coeff_modulus();
//            for(int i=0;i<coeff.size();i++)
//            {
//                cout<<"seal ring ckks inner coeff_modules "<<log2(coeff[i].value())<<endl;
//            }
            for (size_t i = 0; i < this->ciphertexts.size(); i++) {
                encoders[i]->encode(r.get_poly().get_limb(i),pow(2.0,48),ptxt);
                try {
                    //cout<<"*= scale before"<<log2(this->ciphertexts[i].scale())<<" "<<log2(ptxt.scale())<<endl;
                    evaluators[i]->multiply_plain_inplace(this->ciphertexts[i], ptxt);

                    //明文*密文 rescale
                    evaluators[i]->rescale_to_next_inplace(this->ciphertexts[i]);
