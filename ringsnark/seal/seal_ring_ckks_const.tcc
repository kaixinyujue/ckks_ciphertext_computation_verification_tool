...
namespace ringsnark::seal {
    ...
    std::vector<EncodingElem> EncodingElem::encode(
            const PublicKey &pk, const std::vector<RingElem> &rs) {
        assert(get_contexts().size() == pk.size());
...
//            KeyGenerator keygen(get_contexts()[i]);
//            PublicKey public_key;
//            keygen.create_public_key(public_key);
            encryptors.push_back(new ::seal::Encryptor(get_contexts()[i], pk[i]));
        }
    ...
    shared(rs, encs, encryptors, ::seal::parms_id_zero)
        for (int i = 0; i < rs.size(); i++) {
            ...
            for (size_t i = 0; i < get_contexts().size(); i++) {
                ...
                encryptors[i]->encrypt(ptxt, ctxts[i]);
            }
  ...
  for (size_t i = 1; i < EncodingElem::get_contexts().size(); i++) {
    ...
    RingElem EncodingElem::decode(const SecretKey &sk, const EncodingElem &e) {
        ...
        for (size_t i = 0; i < e.ciphertexts.size(); i++) {
            ...
            try {
//                cout<<"debug seal ring ckks 0"<<endl;
...
//                }
    ...
    EncodingElem &EncodingElem::operator+=(const EncodingElem &other) {
        ...
        for (size_t i = 0; i < this->ciphertexts.size(); i++) {
            ...
            try {
                //rescale to same scaler
                this->ciphertexts[i].scale()=other.ciphertexts[i].scale();
                //to same level
                ::seal::Ciphertext ctxt=other.ciphertexts[i];
                auto index1=get_contexts()[i].get_context_data(this->ciphertexts[i].parms_id())->chain_index();
                auto index2=get_contexts()[i].get_context_data(ctxt.parms_id())->chain_index();
                 if(index1>=index2){
                  evaluators[i]->mod_switch_to_inplace(this->ciphertexts[i],ctxt.parms_id());
                }else{
                   evaluators[i]->mod_switch_to_inplace(ctxt,this->ciphertexts[i].parms_id());
                 }
                //cout<<"index1 "<<index1<<endl;
               // cout<<"index2 "<<index2<<endl;
                //cout<<endl;
                evaluators[i]->add_inplace(this->ciphertexts[i], ctxt);
    ...
    EncodingElem &EncodingElem::operator*=(const RingElem &r) {
        ...
        if (r.is_scalar()) {
            ...
            for (size_t i = 0; i < this->ciphertexts.size(); i++) {
                ...
                try {
                    auto index1=get_contexts()[i].get_context_data(this->ciphertexts[i].parms_id())->chain_index();
                    auto index2=get_contexts()[i].get_context_data(ptxt.parms_id())->chain_index();
                    if(index1>=index2){
                        evaluators[i]->mod_switch_to_inplace(this->ciphertexts[i],ptxt.parms_id());
                    }else{
                        evaluators[i]->mod_switch_to_inplace(ptxt,this->ciphertexts[i].parms_id());
                    }
