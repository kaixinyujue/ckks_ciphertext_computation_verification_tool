...
#ifndef RINGSNARK_SEAL_RING_CKKS_H
#define RINGSNARK_SEAL_RING_CKKS_H
...
//        [[nodiscard]] Scalar &get_scalar();
        ...
        explicit RingElem( polytools::SealPoly &poly);
        ...
        static RingElem random_exceptional_element(
                 shared_ptr<evaluation_domain<RingElem>> domain = nullptr) {
        ...
        [[nodiscard]] bool fast_is_zero() ;

        [[nodiscard]] bool is_poly() ;

        [[nodiscard]] bool is_scalar() ;

        void negate_inplace();

        inline RingElem operator-()  {
            RingElem res(*this);
            res.negate_inplace();
            return res;
        }

        [[nodiscard]] bool is_invertible()  noexcept;

        void invert_inplace();

        [[nodiscard]] inline RingElem inverse()  {
            RingElem res(*this);
            res.invert_inplace();
            return res;
        }

        RingElem &operator+=( RingElem &other);
...
//        [[nodiscard]] Poly &get_poly();
    ...
    inline RingElem operator+( RingElem &lhs,  RingElem &rhs) {
        RingElem res(lhs);
        res += rhs;
        return res;
    }
    ...
    inline bool operator!=( RingElem &lhs,  RingElem &rhs) {
    ...
    protected:
        ...
        inline static std::vector<::seal::CKKSEncoder *> encoders;
    ...
    public:
        ...
        public:
            ...
            explicit decoding_error( string &msg)
        ...
        EncodingElem( EncodingElem &other) : ciphertexts(other.ciphertexts) {
            assert(!other.ciphertexts.empty());
        }

        [[nodiscard]] bool is_empty()  { return this->ciphertexts.empty(); }
        ...
        static void set_contexts(const vector<::seal::SEALContext> &contexts_) {
            if (contexts.empty()) {
                ...
                encoders = vector<::seal::CKKSEncoder *>();
                evaluators = vector<::seal::Evaluator *>();
                for (const ::seal::SEALContext &c : contexts) {
                    encoders.push_back(new ::seal::CKKSEncoder(c));
        ...
        // for efficiency
        static std::vector<EncodingElem> encode( SecretKey &sk,
                                                 std::vector<RingElem> &rs);
        ...
        static RingElem decode( SecretKey &sk,  EncodingElem &e);
        ...
        [[nodiscard]] static size_t size_in_bits_pk( PublicKey &pk) { return 0; }

        [[nodiscard]] static size_t size_in_bits_sk( SecretKey &sk) {
            size_t size = 0;
        ...
        EncodingElem &operator+=(EncodingElem &other);

        EncodingElem &operator*=(RingElem &other);
        ...
        friend bool operator==(EncodingElem &lhs, EncodingElem &rhs);
    };

    inline EncodingElem operator+(EncodingElem &lhs,
                                  EncodingElem &rhs) {
    ...
    inline EncodingElem operator*( EncodingElem &lhs, RingElem &rhs) {
        EncodingElem res(lhs);
        res *= rhs;
        return res;
    }

    inline EncodingElem operator*(RingElem &lhs, EncodingElem &rhs) {
        EncodingElem res(rhs);
        res *= lhs;
        return res;
    }

    inline bool operator==(EncodingElem &lhs, EncodingElem &rhs) {
...
namespace std {
    ...
    struct hash<ringsnark::seal::RingElem> {
        size_t operator()( ringsnark::seal::RingElem &r)  {
            return r.hash();
        }
    };
}  // namespace std
...
#include "seal_ring_ckks.tcc"

#endif //RINGSNARK_SEAL_RING_CKKS_H