...
namespace seal
{
    ...
    void Ciphertext::load_members(const SEALContext &context, istream &stream, SEAL_MAYBE_UNUSED SEALVersion version)
    {
        ...
        try
        {
            ...
            // computations.
            // std::cout<<"ciphertext_cpp_debug1"<<std::endl;
            if (!is_metadata_valid_for(new_data, context, true))
            {
                // std::cout<<"ciphertext_cpp_debug2"<<std::endl;
                throw logic_error("ciphertext data is invalid");
            }
            // std::cout<<"ciphertext_cpp_debug3"<<std::endl;
            ...
            // std::cout<<"ciphertext_cpp_debug4"<<std::endl;
            // Verify that the buffer is correct
            if (!is_buffer_valid(new_data))
            {
                // std::cout<<"ciphertext_cpp_debug5"<<std::endl;
                throw logic_error("ciphertext data is invalid");
            }
            // std::cout<<"ciphertext_cpp_debug6"<<std::endl;
        }
        catch (const ios_base::failure &)
        {
            stream.exceptions(old_except_mask);
