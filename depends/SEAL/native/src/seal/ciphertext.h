    ...
    public:
        ...
        */
        inline std::streamoff load(const SEALContext &context, std::istream &stream)
        {
            ...
            // std::cout<<"load_cipher_debug1"<<std::endl;
            auto in_size = new_data.unsafe_load(context, stream);
            // std::cout<<"load_cipher_debug2"<<std::endl;
            if (!is_valid_for(new_data, context))
            {
                // std::cout<<"load_cipher_debug3"<<std::endl;
                throw std::logic_error("ciphertext data is invalid");
            }
            // std::cout<<"load_cipher_debug4"<<std::endl;
            std::swap(*this, new_data);
            // std::cout<<"load_cipher_debug5"<<std::endl;
            return in_size;
        }
        ...
        */
        ...
        void my_save(std::string filePath) const{
            ...
            std::uint64_t pa, pb, pc, pd, cf;
            pa = parms_id_[0];
            pb = parms_id_[1];
            pc = parms_id_[2];
            pd = parms_id_[3];
            cf = correction_factor_;
            std::ofstream ofs(filePath + "pa_cf.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ofs) {
                std::cerr << "Error opening file for writing: " << filePath + "pa_cf.bin" << std::endl;
                return;
            }
            ofs.write(reinterpret_cast<const char*>(&pa), sizeof(std::uint64_t));
            ofs.write(reinterpret_cast<const char*>(&pb), sizeof(std::uint64_t));
            ofs.write(reinterpret_cast<const char*>(&pc), sizeof(std::uint64_t));
            ofs.write(reinterpret_cast<const char*>(&pd), sizeof(std::uint64_t));
            ofs.write(reinterpret_cast<const char*>(&cf), sizeof(std::uint64_t));
            ofs.close();


            std::ofstream ofs1(filePath + "bo_du.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ofs1) {
                std::cerr << "Error opening file for writing: " << filePath + "bo_du.bin" << std::endl;
                return;
            }
            double sc = scale_;
            double isN = 0;
            ...
            ofs1.write(reinterpret_cast<const char*>(&sc), sizeof(double));
            ofs1.write(reinterpret_cast<const char*>(&isN), sizeof(double));
            ofs1.close();


            std::ofstream ofs2(filePath + "size_t.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ofs2) {
                std::cerr << "Error opening file for writing: " << filePath + "size_t.bin" << std::endl;
                return;
            }
            size_t this_size = size_;
            size_t this_degree = poly_modulus_degree_;
            size_t this_coeff_size = coeff_modulus_size_;
            ofs2.write(reinterpret_cast<const char*>(&this_size), sizeof(size_t));
            ofs2.write(reinterpret_cast<const char*>(&this_degree), sizeof(size_t));
            ofs2.write(reinterpret_cast<const char*>(&this_coeff_size), sizeof(size_t));
            ofs2.close();


            std::ofstream ofs3(filePath + "data_size.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ofs3) {
                std::cerr << "Error opening file for writing: " << filePath + "data_size.bin" << std::endl;
                return;
            }
            size_t data_siz = data_.size();
            ofs3.write(reinterpret_cast<const char*>(&data_siz), sizeof(size_t));
            ofs3.close();

            std::ofstream ofs4(filePath + "data.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ofs4) {
                std::cerr << "Error opening file for writing: " << filePath + "data.bin" << std::endl;
                return;
            }
            for(size_t i=0;i<data_siz;i++){
                uint64_t datmp = data_[i];
                ofs4.write(reinterpret_cast<const char*>(&datmp), sizeof(uint64_t));
            }
            ofs4.close();
        }

        void my_load(std::string filePath){
            std::uint64_t pa, pb, pc, pd, cf;
            std::ifstream ifs(filePath + "pa_cf.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ifs) {
                std::cerr << "Error opening file for reading: " << filePath + "pa_cf.bin" << std::endl;
                return;
            }
            ifs.read(reinterpret_cast<char*>(&pa), sizeof(std::uint64_t));
            ifs.read(reinterpret_cast<char*>(&pb), sizeof(std::uint64_t));
            ifs.read(reinterpret_cast<char*>(&pc), sizeof(std::uint64_t));
            ifs.read(reinterpret_cast<char*>(&pd), sizeof(std::uint64_t));
            ifs.read(reinterpret_cast<char*>(&cf), sizeof(std::uint64_t));
            ifs.close();

            std::array<std::uint64_t, 4> params_id010 = {{pa, pb, pc, pd}};
            parms_id_ = params_id010;
            correction_factor_ = cf;


            std::ifstream ifs1(filePath + "bo_du.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ifs1) {
                std::cerr << "Error opening file for reading: " << filePath + "bo_du.bin" << std::endl;
                return;
            }
            double sc, isN;
            ifs1.read(reinterpret_cast<char*>(&sc), sizeof(double));
            ifs1.read(reinterpret_cast<char*>(&isN), sizeof(double));
            ifs1.close();
            if(isN==0){
                is_ntt_form_ = false;
            }
            ...
            std::ifstream ifs2(filePath + "size_t.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ifs2) {
                std::cerr << "Error opening file for reading: " << filePath + "size_t.bin" << std::endl;
                return;
            }
            size_t this_size, this_degree, this_coeff_size;
            ifs2.read(reinterpret_cast<char*>(&this_size), sizeof(size_t));
            ifs2.read(reinterpret_cast<char*>(&this_degree), sizeof(size_t));
            ifs2.read(reinterpret_cast<char*>(&this_coeff_size), sizeof(size_t));
            ifs2.close();
            size_ = this_size;
            poly_modulus_degree_ = this_degree;
            coeff_modulus_size_ = this_coeff_size;


            std::ifstream ifs3(filePath + "data_size.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ifs3) {
                std::cerr << "Error opening file for reading: " << filePath + "data_size.bin" << std::endl;
                return;
            }
            size_t data_siz;
            ifs3.read(reinterpret_cast<char*>(&data_siz), sizeof(size_t));
            ifs3.close();

            data_.resize(data_siz);
            std::ifstream ifs4(filePath + "data.bin", std::ios::binary); // 以二进制方式打开文件
            if (!ifs4) {
                std::cerr << "Error opening file for reading: " << filePath + "data.bin" << std::endl;
                return;
            }
            for(size_t i=0;i<data_siz;i++){
                uint64_t datmp;
                ifs4.read(reinterpret_cast<char*>(&datmp), sizeof(uint64_t));
                data_[i] = datmp;
            }
            ifs4.close();
        }
