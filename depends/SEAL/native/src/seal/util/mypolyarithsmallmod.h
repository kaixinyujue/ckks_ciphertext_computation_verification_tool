...
// add 使用
                ...
                RNSIter result)
        {
            ...
            SEAL_ITERATE(iter(operand1, operand2, modulus, result), coeff_modulus_size, [&](auto I) {
                add_poly_coeffmod1(get<0>(I), get<1>(I), poly_modulus_degree, get<2>(I), get<3>(I));
            });
        }
                ...
                ConstPolyIter operand1, ConstPolyIter operand2, std::size_t size, ConstModulusIter modulus, PolyIter result)
        {
            ...
            SEAL_ITERATE(iter(operand1, operand2, result), size, [&](auto I) {
                add_poly_coeffmod1(get<0>(I), get<1>(I), coeff_modulus_size, modulus, get<2>(I));
            });
        }

        void sub_poly_coeffmod1(
                ConstCoeffIter operand1, ConstCoeffIter operand2, std::size_t coeff_count, const Modulus &modulus,
                CoeffIter result);

        inline void sub_poly_coeffmod1(
                ConstRNSIter operand1, ConstRNSIter operand2, std::size_t coeff_modulus_size, ConstModulusIter modulus,
                RNSIter result)
        {
#ifdef SEAL_DEBUG
            ...
            auto poly_modulus_degree = result.poly_modulus_degree();
            SEAL_ITERATE(iter(operand1, operand2, modulus, result), coeff_modulus_size, [&](auto I) {
                sub_poly_coeffmod1(get<0>(I), get<1>(I), poly_modulus_degree, get<2>(I), get<3>(I));
            });
        }
                ...
                ConstPolyIter operand1, ConstPolyIter operand2, std::size_t size, ConstModulusIter modulus, PolyIter result)
        {
            ...
            auto coeff_modulus_size = result.coeff_modulus_size();
            SEAL_ITERATE(iter(operand1, operand2, result), size, [&](auto I) {
                sub_poly_coeffmod1(get<0>(I), get<1>(I), coeff_modulus_size, modulus, get<2>(I));
            });
        }
        ...
        */
        void add_poly_scalar_coeffmod1(
        ...
        @param[in] scalar Must be less than modulus.value().
        */
        inline void add_poly_scalar_coeffmod1(
                ...
                RNSIter result)
        {
            ...
            auto poly_modulus_degree = result.poly_modulus_degree();
            SEAL_ITERATE(iter(poly, modulus, result), coeff_modulus_size, [&](auto I) {
                add_poly_scalar_coeffmod1(get<0>(I), poly_modulus_degree, scalar, get<1>(I), get<2>(I));
            });
        }
        ...
        */
                ...
                ConstPolyIter poly_array, std::size_t size, std::uint64_t scalar, ConstModulusIter modulus, PolyIter result)
        {
            ...
            auto coeff_modulus_size = result.coeff_modulus_size();
            SEAL_ITERATE(iter(poly_array, result), size, [&](auto I) {
                add_poly_scalar_coeffmod1(get<0>(I), coeff_modulus_size, scalar, modulus, get<1>(I));
            });
        }
        ...
        */
        //sub
        void sub_poly_scalar_coeffmod1(
                ConstCoeffIter poly, std::size_t coeff_count, std::uint64_t scalar, const Modulus &modulus,
                CoeffIter result);
        ...
        @param[in] scalar Must be less than modulus.value().
        */
        inline void sub_poly_scalar_coeffmod1(
                ConstRNSIter poly, std::size_t coeff_modulus_size, std::uint64_t scalar, ConstModulusIter modulus,
                RNSIter result)
        {
#ifdef SEAL_DEBUG
            ...
            auto poly_modulus_degree = result.poly_modulus_degree();
            SEAL_ITERATE(iter(poly, modulus, result), coeff_modulus_size, [&](auto I) {
                sub_poly_scalar_coeffmod1(get<0>(I), poly_modulus_degree, scalar, get<1>(I), get<2>(I));
            });
        }
        ...
        */
                ...
                ConstPolyIter poly_array, std::size_t size, std::uint64_t scalar, ConstModulusIter modulus, PolyIter result)
        {
            ...
            auto coeff_modulus_size = result.coeff_modulus_size();
            SEAL_ITERATE(iter(poly_array, result), size, [&](auto I) {
                sub_poly_scalar_coeffmod1(get<0>(I), coeff_modulus_size, scalar, modulus, get<1>(I));
            });
        }
// multi_scalar 也会调用这个
        void multiply_poly_scalar_coeffmod1(
                ConstCoeffIter poly, std::size_t coeff_count, MultiplyUIntModOperand scalar, const Modulus &modulus,
                CoeffIter result);

        inline void multiply_poly_scalar_coeffmod1(
                ConstCoeffIter poly, std::size_t coeff_count, std::uint64_t scalar, const Modulus &modulus,
                CoeffIter result)
        {
            ...
            multiply_poly_scalar_coeffmod1(poly, coeff_count, temp_scalar, modulus, result);
        }
                ...
                RNSIter result)
        {
            ...
            auto poly_modulus_degree = result.poly_modulus_degree();
            SEAL_ITERATE(iter(poly, modulus, result), coeff_modulus_size, [&](auto I) {
                multiply_poly_scalar_coeffmod1(get<0>(I), poly_modulus_degree, scalar, get<1>(I), get<2>(I));
            });
        }
                ...
                ConstPolyIter poly_array, std::size_t size, std::uint64_t scalar, ConstModulusIter modulus, PolyIter result)
        {
            ...
            auto coeff_modulus_size = result.coeff_modulus_size();
            SEAL_ITERATE(iter(poly_array, result), size, [&](auto I) {
                multiply_poly_scalar_coeffmod1(get<0>(I), coeff_modulus_size, scalar, modulus, get<1>(I));
            });
        }
//multiply_inplace
        void dyadic_product_coeffmod1(
                ConstCoeffIter operand1, ConstCoeffIter operand2, std::size_t coeff_count, const Modulus &modulus,
                CoeffIter result);

        inline void dyadic_product_coeffmod1(
                ConstRNSIter operand1, ConstRNSIter operand2, std::size_t coeff_modulus_size, ConstModulusIter modulus,
                RNSIter result)
        {
#ifdef SEAL_DEBUG
            ...
            auto poly_modulus_degree = result.poly_modulus_degree();
            SEAL_ITERATE(iter(operand1, operand2, modulus, result), coeff_modulus_size, [&](auto I) {
                dyadic_product_coeffmod1(get<0>(I), get<1>(I), poly_modulus_degree, get<2>(I), get<3>(I));
            });
        }
                ...
                ConstPolyIter operand1, ConstPolyIter operand2, std::size_t size, ConstModulusIter modulus, PolyIter result)
        {
            ...
            auto coeff_modulus_size = result.coeff_modulus_size();
            SEAL_ITERATE(iter(operand1, operand2, result), size, [&](auto I) {
                dyadic_product_coeffmod1(get<0>(I), get<1>(I), coeff_modulus_size, modulus, get<2>(I));
            });
        }
//
...
//            multiply_poly_scalar_coeffmod1(poly, coeff_count, mono_coeff, modulus, temp);
