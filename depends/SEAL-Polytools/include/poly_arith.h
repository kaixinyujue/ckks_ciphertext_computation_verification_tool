...
namespace polytools
{
    ...
    public:
        ...
        //获取
        std::vector<uint64_t> get_coeff_list(std::vector<uint64_t> mod)
        {
            size_t count=mod.size();
            size_t coeff_count=get_coeff_count();
            size_t modulus_count=get_coeff_modulus_count();
            size_t len=coeff_count/2 *count;
            std::vector<uint64_t> res(len);
            size_t k=0;
            for(size_t i = 0; i < count; i++){
                for(size_t j = 0;j < coeff_count; j=j+2){
                    res[k++] = data[i * coeff_count + j] % mod[i];
                }
            }
            return res;
        }
